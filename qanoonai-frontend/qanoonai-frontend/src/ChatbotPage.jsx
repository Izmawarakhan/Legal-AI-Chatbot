...
const API_BASE = import.meta.env.VITE_CHATBOT_URL || "http://localhost:8000";
...
export default function ChatbotPage() {
  ...
  // ✅ Auto-speak AI response ONLY when message was sent via mic
  ...
  const fetchSessions = async () => {
    try {
      ...
      const url = userId
        ? `${API_BASE}/api/chat/sessions?user_id=${userId}`
      ...
      >
        ...
        <div style={{ padding: "16px 20px", borderTop: `1px solid ${colors.border}`, fontSize: 11, color: colors.textMuted }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span>Guidance only. Consult a licensed lawyer.</span>
          </div>
        </div>
      </div>
        ...
        {isDragOver && (
          <div
            style={{
              ...
              borderRadius: 16,
              display: "flex", alignItems: "center", justifyValue: "center",
              ...
              {(() => {
                ...
                return (
                  <button
                    onClick={() => sendMessage()}
                    disabled={!canSend}
                    title={stillUploading ? "Waiting for file..." : "Send message"}
                    style={{

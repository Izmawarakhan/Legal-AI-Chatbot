...
- "Aap court mein case file kar sakti/sakte hain"
...
EMPATHY RULES:
- Say "I understand this is difficult" or "Main samajh sakti/sakta hoon" ONLY ONCE in the entire conversation, in your first response.
- NEVER repeat empathy phrases like "I understand", "I can see", "Main samajh sakti hoon", "Yeh mushkil waqt hai" in later messages.
...
- You are a FEMALE AI legal consultant named QanoonAI.
- In Roman Urdu, ALWAYS use feminine verb forms for yourself: "sakti hoon", "chahti hoon", "karungi", "samajh sakti hoon", "batati hoon", "poochti hoon".
- NEVER use masculine forms for yourself: "sakta hoon", "chahta hoon", "karunga", "samajh sakta hoon", "batata hoon", "poochta hoon".
- Examples of CORRECT self-reference: "Main aapki madad kar sakti hoon", "Main samajh sakti hoon", "Main aapko batati hoon", "Main ye poochna chahti hoon"
- Examples of WRONG self-reference: "Main aapki madad kar sakta hoon", "Main samajh sakta hoon", "Main aapko batata hoon"
- For the USER, use gender-neutral forms or ask their preference. If unknown, use "aap" (formal) which is gender-neutral.
...
def get_conversation_chain(vector_store: FAISS, api_key: str, religion: str):
    """Creates the conversational retrieval chain with smart category-aware retriever"""
    llm = ChatGroq(
    ...
    retriever = SmartCategoryRetriever(
...
# ============================================
# SMART CATEGORY-AWARE RETRIEVER
# ============================================
# Problem: Criminal Law has 47% of all chunks, so it dominates
# every query. Smaller categories (Health 1.2%, Land 1%, Islamic 0.2%)
# never get retrieved.
#
# Solution: Detect likely category from query keywords FIRST,
# then retrieve more docs (k*4) and filter/boost matching category.
# ============================================
from langchain_core.retrievers import BaseRetriever
from langchain_core.callbacks import CallbackManagerForRetrieverRun

# Keywords for category detection from user query
QUERY_CATEGORY_KEYWORDS = {
    "Family Law": [
        "divorce", "khula", "custody", "meher", "mehr", "marriage", "nikah",
        "wife", "husband", "spouse", "children", "child", "maintenance",
        "dower", "family court", "guardian", "talaq", "iddat", "biwi",
        "shohar", "bacche", "shaadi", "rishta", "dissolution",
        "second marriage", "polygamy", "haq mehr",
    ],
    "Criminal Law": [
        "fir", "police", "murder", "theft", "robbery", "snatch", "bail",
        "arrest", "fraud", "assault", "violence", "kidnap", "rape",
        "ppc", "crpc", "penal code", "criminal", "accused", "complaint",
        "challan", "weapon", "hurt", "qatl", "chori", "loot", "daku",
        "attack", "threatening", "extortion",
    ],
    "Labour Laws": [
        "salary", "termination", "fired", "employer", "employee", "company",
        "wages", "harassment", "workplace", "factory", "factories act",
        "overtime", "leave", "dismissal", "appointment", "contract",
        "standing orders", "industrial", "labour", "labor",
        "naukri", "tankhwah", "mulazim",
    ],
    "Land & Property Laws": [
        "property", "land", "plot", "house", "mutation", "registry",
        "deed", "transfer", "patwari", "tehsildar", "inheritance",
        "succession", "encroachment", "ownership", "tenant", "rent",
        "zameen", "ghar", "jaidad", "qabza", "intiqal",
    ],
    "Islamic Religious Laws": [
        "hudood", "zina", "blasphemy", "waqf", "auqaf", "shariat",
        "zakat", "islamic", "295", "qazf", "religious", "shariah",
        "fatwa", "mufti", "maulvi", "masjid", "mosque", "toheen",
    ],
    "Excise Taxation Laws": [
        "tax", "fbr", "income tax", "sales tax", "customs", "excise",
        "ntn", "strn", "refund", "assessment", "tribunal", "revenue",
        "taxation", "duty", "tariff", "return", "challan",
        "tax notice", "appeal",
    ],
    "Health & Medical Laws": [
        "hospital", "doctor", "medical", "negligence", "malpractice",
        "treatment", "patient", "surgery", "pmdc", "drug", "medicine",
        "pharmacy", "nurse", "clinic", "emergency", "ambulance",
        "dawai", "ilaaj", "tabib", "operation",
    ],
}


def detect_category_from_query(query: str) -> Optional[str]:
    """Detect the most likely law category from query keywords"""
    query_lower = query.lower()
    
    scores = {}
    for cat_name, keywords in QUERY_CATEGORY_KEYWORDS.items():
        score = 0
        for kw in keywords:
    ...
    # Only return if there's a clear signal (score >= 1)
    if best_score >= 1:
        return best_cat
...
class SmartCategoryRetriever(BaseRetriever):
    """Custom retriever that detects category from query and filters results"""
    ...
    def _get_relevant_documents(
        ...
        """Retrieve docs with category-aware filtering"""
        
        # Step 1: Detect category from query
        detected_cat = detect_category_from_query(query)
        
        if detected_cat:
            # Step 2: Retrieve MORE docs than needed (k * 6)
            # so we have enough to filter from
            all_docs = self.vector_store.similarity_search(query, k=self.k * 6)
            
            # Step 3: Separate into matching and non-matching
            matching = [d for d in all_docs if d.metadata.get("category") == detected_cat]
            others = [d for d in all_docs if d.metadata.get("category") != detected_cat]
            
            # Step 4: Take matching docs first, fill remaining with others
            # Aim for at least 60% from the correct category
            min_matching = max(3, int(self.k * 0.6))  # At least 3 from correct category
            
            if len(matching) >= min_matching:
                # Good — we have enough matching docs
                result = matching[:self.k]
                # Fill remaining slots with other docs for diversity
                remaining = self.k - len(result)
                if remaining > 0:
                    result.extend(others[:remaining])
            else:
                # Not enough matching — use what we have + fill with others
                result = matching + others[:self.k - len(matching)]
            
            return result[:self.k]
        
        else:
            # No clear category detected — use default retrieval
